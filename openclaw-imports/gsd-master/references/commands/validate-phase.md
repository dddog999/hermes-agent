...
```bash
AUDITOR_MODEL=$(node "@../scripts/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "@../scripts/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
**State B (create):**
1. Read template from `$HOME/.agents/skills/gsd-master/templates/VALIDATION.md`
...
node "@../scripts/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```

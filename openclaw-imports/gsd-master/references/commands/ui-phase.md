...
```bash
UI_RESEARCHER_MODEL=$(node "@../scripts/gsd-tools.cjs" resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "@../scripts/gsd-tools.cjs" resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "@../scripts/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
Template: $HOME/.agents/skills/gsd-master/templates/UI-SPEC.md
</output>
...
```bash
node "@../scripts/gsd-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```

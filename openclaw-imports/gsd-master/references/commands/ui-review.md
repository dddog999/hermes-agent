...
```bash
UI_AUDITOR_MODEL=$(node "@../scripts/gsd-tools.cjs" resolve-model gsd-ui-auditor --raw)
```
...
```bash
node "@../scripts/gsd-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```

...
```bash
INIT=$(node "@../scripts/gsd-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "@../scripts/gsd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
node "@../scripts/gsd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "@../scripts/gsd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "@../scripts/gsd-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "@../scripts/gsd-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```

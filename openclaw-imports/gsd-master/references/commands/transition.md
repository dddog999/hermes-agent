...
```bash
TRANSITION=$(node "@../scripts/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "@../scripts/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "@../scripts/gsd-tools.cjs" config-set workflow._auto_chain_active false
```

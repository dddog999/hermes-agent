...
```bash
INIT=$(node "@../scripts/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
node "@../scripts/gsd-tools.cjs" roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "@../scripts/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "@../scripts/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "@../scripts/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "@../scripts/gsd-tools.cjs" verify key-links "$plan")
...
See $HOME/.agents/skills/gsd-master/templates/verification-report.md for complete template.
</step>

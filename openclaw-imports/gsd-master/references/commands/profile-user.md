...
Key references:
- @$HOME/.agents/skills/gsd-master/references/ui-brand.md (display patterns)
- @$HOME/.agents/skills/gsd-master/agents/gsd-user-profiler.md (profiler agent definition)
- @$HOME/.agents/skills/gsd-master/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.agents/skills/gsd-master/USER-PROFILE.md"
...
```bash
cp "$HOME/.agents/skills/gsd-master/USER-PROFILE.md" "$HOME/.agents/skills/gsd-master/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.agents/skills/gsd-master/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node @../scripts/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node @../scripts/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `$HOME/.agents/skills/gsd-master/references/user-profiling.md`
...
Reference: @$HOME/.agents/skills/gsd-master/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node @../scripts/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node @../scripts/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node @../scripts/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.agents/skills/gsd-master/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.agents/skills/gsd-master/USER-PROFILE.md" and jump to step 10.
...
```bash
node @../scripts/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node @../scripts/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node @../scripts/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH="$HOME/.agents/skills/gsd-master/USER-PROFILE.backup.md"
```
...
Your profile:    $HOME/.agents/skills/gsd-master/USER-PROFILE.md
```

...
```bash
INIT=$(node "@../scripts/gsd-tools.cjs" init phase-op "${target}")
...
```bash
RESULT=$(node "@../scripts/gsd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "@../scripts/gsd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "@../scripts/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```

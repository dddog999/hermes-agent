...
```bash
ROADMAP=$(node "@../scripts/gsd-tools.cjs" roadmap analyze)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "@../scripts/gsd-tools.cjs" summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node "@../scripts/gsd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `$HOME/.agents/skills/gsd-master/templates/retrospective.md`.
...
**Commit:**
```bash
node "@../scripts/gsd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "@../scripts/gsd-tools.cjs" init execute-phase "1")
...
```bash
node "@../scripts/gsd-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```

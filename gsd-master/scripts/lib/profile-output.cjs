...
function cmdWriteProfile(cwd, options, raw) {
  ...
  const templatePath = path.join(SKILL_ROOT, 'templates', 'user-profile.md');
  ...
  if (!outputPath) {
    outputPath = path.join(SKILL_ROOT, 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  const templatePath = path.join(SKILL_ROOT, 'templates', 'dev-preferences.md');
  if (!fs.existsSync(templatePath)) error(`Template not found: ${templatePath}`);
  let template = fs.readFileSync(templatePath, 'utf-8');

...
// ─── Determine Skill Root Path ──────────────────────────────────────────────
// Dual runtime support: Claude Code uses ~/.agents/skills/, Hermes uses ~/.hermes/skills/
// Priority: GSD_MASTER_PATH env var > __dirname (auto-detect) > .hermes > .agents
let SKILL_ROOT;
const homeDir = process.env.HOME || process.env.USERPROFILE;

// Env var override (for sandboxed environments)
if (process.env.GSD_MASTER_PATH) {
  SKILL_ROOT = path.resolve(process.env.GSD_MASTER_PATH);
} else if (homeDir) {
  // Auto-detect: prefer .hermes (Hermes runtime), fall back to .agents (Claude Code runtime)
  const hermesRoot = path.join(homeDir, '.hermes', 'skills', 'gsd-master');
  const agentsRoot = path.join(homeDir, '.agents', 'skills', 'gsd-master');
  if (fs.existsSync(path.join(hermesRoot, 'scripts', 'lib'))) {
    SKILL_ROOT = hermesRoot;
  } else if (fs.existsSync(path.join(agentsRoot, 'scripts', 'lib'))) {
    SKILL_ROOT = agentsRoot;
  } else {
    // Last resort: use __dirname (works if script is invoked from skill directory)
    SKILL_ROOT = path.resolve(__dirname, '..');
  }
} else {
  // Fallback to __dirname if HOME not set
  SKILL_ROOT = path.resolve(__dirname, '..');
}

// Normalize path separators for cross-platform compatibility
SKILL_ROOT = SKILL_ROOT.replace(/\\/g, '/');

// Validate that we're in the right place
const libDir = path.join(SKILL_ROOT, 'scripts', 'lib').replace(/\\/g, '/');
const refsDir = path.join(SKILL_ROOT, 'references').replace(/\\/g, '/');
if (!fs.existsSync(libDir) || !fs.existsSync(refsDir)) {
  console.error(`Warning: Could not find gsd-master at ${SKILL_ROOT}`);
  console.error(`  lib exists: ${fs.existsSync(libDir)} (${libDir})`);
  console.error(`  references exists: ${fs.existsSync(refsDir)} (${refsDir})`);
  console.error('Set GSD_MASTER_PATH environment variable to the correct path.');
  process.exit(1);
}

// Now require modules using absolute paths
const { error } = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'core.cjs'));
const state = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'state.cjs'));
const phase = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'phase.cjs'));
const roadmap = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'roadmap.cjs'));
const verify = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'verify.cjs'));
const config = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'config.cjs'));
const template = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'template.cjs'));
const milestone = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'milestone.cjs'));
const commands = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'commands.cjs'));
const init = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'init.cjs'));
const frontmatter = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'frontmatter.cjs'));
const profilePipeline = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'profile-pipeline.cjs'));
const profileOutput = require(path.join(SKILL_ROOT, 'scripts', 'lib', 'profile-output.cjs'));

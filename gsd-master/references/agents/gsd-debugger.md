...
```
agent_file = read_file("references/agents/gsd-debugger.md")
delegate_task(
  goal=f"Diagnose and fix: {bug_description}. Find root cause, fix, verify. Write DEBUG.md report.",
  context=f"{agent_file}\n\nBug description: {user_description}\nProject paths:\n- .planning/STATE.md\n- ./CLAUDE.md (if exists)\n- Relevant source files: {affected_files}"
)
```
...
- DEBUG.md created in phase or project dir
- STATE.md blocker removed if fixed

**Mode awareness:** If `--mode analyze`: find cause only. If `--mode fix`: also fix and verify.
...
Spawned by `/gsd:debug` command or via Hermes `delegate_task`.
...
```bash
INIT=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" state load)
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```

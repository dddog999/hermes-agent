...
**How to spawn:**
```
agent_file = read_file("references/agents/gsd-plan-checker.md")
delegate_task(
  goal="Verify PLAN.md will achieve phase goal. Goal-backward analysis of plan quality.",
  context=f"{agent_file}\n\nProject paths: {plan_path}, .planning/ROADMAP.md, .planning/REQUIREMENTS.md"
)
```

**Available tools:** Read, Bash, Glob, Grep

**Expected output:** Plan quality verdict (APPROVED / NEEDS_REVISION), specific gaps identified.
...
```bash
INIT=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "./skills/gsd-master/scripts/gsd-tools.cjs" roadmap get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```

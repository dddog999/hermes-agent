...
**How to spawn:**
```
agent_file = read_file("references/agents/gsd-ui-checker.md")
delegate_task(
  goal="Validate UI-SPEC.md design contracts against 6 quality dimensions. Produce BLOCK/FLAG/PASS verdicts.",
  context=f"{agent_file}\n\nProject paths: {phase_dir}/UI-SPEC.md, UI source files"
)
```

**Available tools:** Read, Bash, Glob, Grep

**Expected output:** BLOCK/FLAG/PASS verdicts for each UI-SPEC contract item.

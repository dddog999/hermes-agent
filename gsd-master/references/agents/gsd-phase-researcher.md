...
**How to spawn:**
```
agent_file = read_file("references/agents/gsd-phase-researcher.md")
delegate_task(
  goal="Research how to implement Phase {N}. Produce RESEARCH.md for the planner.",
  context=f"{agent_file}\n\nProject paths: .planning/phases/{phase}/context/, .planning/ROADMAP.md, .planning/REQUIREMENTS.md"
)
```

**Available tools:** Read, Write, Bash, Grep, Glob, WebSearch, WebFetch, MCP context7 tools

**Expected output:** RESEARCH.md in phase directory with technical approaches, tradeoffs, references.
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```bash
INIT=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" init phase-op "${PHASE}")
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```

...
```
agent_file = read_file("references/agents/gsd-planner.md")
delegate_task(
  goal=f"Create PLAN.md for Phase {N}. Analyze scope, decompose into 2-3 tasks, identify dependencies, create wave assignments.",
  context=f"{agent_file}\n\nProject paths:\n- Phase dir: {phase_dir}\n- ROADMAP.md: .planning/ROADMAP.md\n- REQUIREMENTS.md: .planning/REQUIREMENTS.md\n- STATE.md: .planning/STATE.md\n- .planning/phases/{phase}/context/ (if exists)"
)
```

**Available tools:** Read, Write, Bash, Glob, Grep, WebFetch, MCP context7 tools

**Expected output:**
- PLAN.md created in phase directory
- 2-3 parallelizable tasks per plan
- Wave 1 tasks have no dependencies
- Must-haves derived via goal-backward from phase goal
- Output format: PLAN.md with `<objective>`, `<tasks>`, `<success_criteria>`, `<verification>`
...
Spawned by `/gsd:plan-phase` orchestrator or via Hermes `delegate_task`.
...
<execution_context>
./skills/gsd-master/scripts/workflows/execute-plan.md
./skills/gsd-master/templates/summary.md
</execution_context>
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "./skills/gsd-master/scripts/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>

...
**How to spawn:**
```
agent_file = read_file("references/agents/gsd-ui-researcher.md")
delegate_task(
  goal="Produce UI-SPEC.md design contract for frontend phase. Read upstream artifacts, detect design system state.",
  context=f"{agent_file}\n\nProject paths: .planning/PROJECT.md, .planning/REQUIREMENTS.md, upstream phase artifacts"
)
```

**Available tools:** Read, Write, Bash, Grep, Glob, WebSearch, WebFetch, MCP context7 tools

**Expected output:** UI-SPEC.md design contract in phase directory.
...
Use template from `./skills/gsd-master/references/templates/UI-SPEC.md`.
...
Read template: `./skills/gsd-master/references/templates/UI-SPEC.md`
...
```bash
node "./skills/gsd-master/scripts/gsd-tools.cjs" commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```

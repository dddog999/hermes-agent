# Jina Reader
> Demoted from `jina-reader` during web-content-extraction umbrella consolidation.
...
- **CRLF line endings** — The `reader.sh` script may have Windows line endings (`\r\n`). If you get `/usr/bin/env: 'bash\r': No such file or directory`, fix with: `tr -d '\r' < reader.sh > reader_fixed.sh && mv reader_fixed.sh reader.sh`
- **jq required** — The script uses `jq` for JSON parsing. Install with `sudo apt install jq` (or equivalent).
- **API key for search** — Read mode works without a key (rate-limited), but search and ground modes require `JINA_API_KEY`. Free tier: 10M tokens at https://jina.ai/reader/
- **WSL TUN proxy issues** — If WSL uses a TUN mode proxy (e.g., Clash TUN), some Jina endpoints may fail with TLS errors.

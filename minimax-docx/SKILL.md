...
| 文件 | 来源 Skill | 主题 |
|------|-----------|------|
| `references/minimax-pdf.md` | `minimax-pdf` (archived) | MiniMax PDF skill: CREATE/FILL/REFORMAT pipelines, typography, print-ready generation |
| `references/minimax-xlsx.md` | `minimax-xlsx` (archived) | MiniMax XLSX skill: READ / CREATE / EDIT / FIX / VALIDATE workflows, OOXML XML templates |
| `references/docx-preserve-drawingml.md` | `docx-preserve-drawingml` (archived) | DOCX DrawingML 文本框/图形引用保留 |
| `references/docx-textbox-preservation.md` | `docx-textbox-preservation` (archived) | DOCX 文本框文字提取/填入 |
| `references/docx-xml-replace.md` | `docx-xml-replace` (archived) | DOCX XML 直接替换模板 |
| `references/docx-xml-crossrun-replace.md` | `docx-xml-crossrun-replace` (archived) | DOCX cross-run（跨段落）标签替换 |
| `references/docx-template-fill-advanced.md` | `docx-template-fill-advanced` (archived) | DOCX 模板高级填充 XPath |
| `references/docx-template-ocr-fill.md` | `docx-template-ocr-fill` (archived) | DOCX OCR 批量生成填充 |

...
## Windows HTTP 服务器：从 WSL 访问 Windows 进程
updated: 2026-05-26

### 问题：Windows 进程正常监听，但 WSL curl 连不上

一个 Windows 上的 Python HTTP 服务器（如 `python server.py`）：
- PowerShell 本机 `Invoke-WebRequest localhost:8765` 正常 ✅
- WSL `curl http://localhost:8765` 超时 ❌
- `Get-NetTCPConnection` 显示 `0.0.0.0:8765 Listen` ✅

**根本原因**：Windows 防火墙阻断了从 WSL（跨 VM）发起的入站 TCP 连接。

### 修复：加防火墙入站规则（限定程序）

```powershell
# 需要管理员权限运行
netsh advfirewall firewall add rule name="rulename" dir=in action=allow protocol=TCP localport=8765 program="C:\Users\...\Python\Python312\python.exe" enable=yes
```

**注意**：
- `program=` 优于只开端口——精确授权该程序，不影响其他进程
- 需要管理员权限运行
- 规则名唯一，重复会报错（先删再加：`netsh advfirewall firewall delete rule name="rulename"`）

**验证**：
```bash
# WSL 内测试局域网 IP（先通过 PowerShell 查本机 IP）
powershell.exe -NoProfile -Command "Get-NetIPAddress -AddressFamily IPv4 -InterfaceIndex (Get-NetAdapter -Physical | Where-Object { $_.Status -eq 'Up' } | Select-Object -First 1).ifIndex | Select-Object -ExpandProperty IPAddress"
curl http://192.168.x.x:8765/your_endpoint
```

### 从 WSL 启动 Windows Python 服务器的正确方式

**❌ 直接调用**（超时或 UNC 路径问题）：
```bash
cmd.exe /c "cd /d C:\path\to && python server.py"
```

**✅ 用 PowerShell Start-Process（最可靠）**：
```powershell
Start-Process -FilePath "python" -ArgumentList "C:\path\to\server.py" -WindowStyle Hidden -PassThru
# 返回对象含 .Id（PID）、.ProcessName
```

**✅ 用 cmd start 后台启动**（适合无窗口长驻进程）：
```powershell
cmd /c "start /b cmd /c python C:\path\to\server.py > C:\path\to\server.log 2>&1"
```

**验证进程和端口**：
```powershell
Get-NetTCPConnection -LocalPort 8765 -ErrorAction SilentlyContinue
# 查进程路径：(Get-Process -Id <PID>).Path
```

### 开机自启：Windows 计划任务

```powershell
$action = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument '-NoProfile -WindowStyle Hidden -File C:\path\to\start_script.ps1'
$trigger = New-ScheduledTaskTrigger -AtLogOn -User "$env:USERNAME"
$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable
$principal = New-ScheduledTaskPrincipal -UserId $env:USERNAME -LogonType Interactive -RunLevel Limited
$task = Register-ScheduledTask -TaskName 'YourTaskName' -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Force
$task | Select-Object TaskName, State, NextRunTime | Format-List
```

**自启脚本推荐写法（幂等检查）**：
```powershell
# 检查端口是否已在监听，是则跳过启动（防止重复进程）
$existing = Get-NetTCPConnection -LocalPort 8765 -ErrorAction SilentlyContinue | Where-Object { $_.State -eq 'Listen' }
if ($existing) {
    Write-Output "[$(Get-Date)] Already running on port $port, skip"
    exit 0
}
Start-Process -FilePath "python" -ArgumentList "C:\path\to\server.py" -WindowStyle Hidden -PassThru
```
...
```powershell
# 查看状态
Get-ScheduledTask -TaskName 'YourTaskName' | Select-Object TaskName, State, NextRunTime
...
| 方案 | 适用场景 | 局限性 |
|------|----------|--------|
| `netsh firewall program=` | 修复已运行的服务器被 WSL 阻 | 需要管理员 |
| `Start-Process` 启动 | 从 WSL 临时启动测试 | 无 stdout 回显 |
| `cmd /c start /b` | 后台长驻（不弹窗） | UNC 路径有时失效 |
| 计划任务 AtLogOn | 开机自启 | 用户首次登录才触发 |